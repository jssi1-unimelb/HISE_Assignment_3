...
--  universe. If removed, it's possible for the index to go out of the range of
--  the "items" array during run time
--
--  Pre  => Item_Count (U) < Max_Items;
--  Ensures the number of items in the universe does not exceed the max number
--  of items. If removed, the number of items in the items array could exceed
--  the item limit, causing a constraint error during runtime due to constraints
--  imposed in the type defintion of ItemArray
--
--------------------------------------------------------------------------------
--  Task 7: Reflection
--
...
procedure Main with SPARK_Mode is
  ...
  function Position_At_Tick (U : Univ.Universe; T : Big_Real) return Boolean
       ...
       (for all I in 1 .. 2 =>
         ...
         = Vector.Add
            (Spatial.To_Vector (Initial_Positions (I)),
             Vector.Scale (Spatial.Vel_To_Vector (Initial_Velocities (I)), T))
  ...
  with
   ...
   Post =>
    ...
    = Vector.Add
       (Spatial.To_Vector (Init),
        Vector.Scale (Spatial.Vel_To_Vector (Vel), T + To_Big_Real (1)));
  ...
  begin
    pragma Assert (Vel_V.X * T + Vel_V.X = Vel_V.X * (T + To_Big_Real (1)));
    pragma Assert (Vel_V.Y * T + Vel_V.Y = Vel_V.Y * (T + To_Big_Real (1)));
    pragma
     Assert
  ...
  --  separated right now.
  procedure Lemma_No_Collision_Pair (U : Univ.Universe; I, J : Integer)
  with
   Ghost,
   Pre  =>
  ...
  procedure Lemma_No_Collision_Pair (U : Univ.Universe; I, J : Integer) is
  ...
  begin
    ...
    Collision_Math.Check_Implies_Safe_Vec (S, V, Pair_Sep2 (I, J), Tick_Count);
  ...
  procedure Reset_Universe with Post => Position_Invariant (U) is
  begin
...
begin
  ...
  for Frame in 1 .. 5000 loop
    ...
    declare
      Old_Tick : constant Big_Real := Tick_Count
      with Ghost;
      Old_U    : constant Univ.Universe := U
      with Ghost;
    begin
      Univ.Tick (U);
      ...
      for Item in 1 .. 2 loop
        pragma Loop_Invariant (Tick_Count = Old_Tick + To_Big_Real (1));
        pragma Loop_Invariant (Position_At_Tick (Old_U, Old_Tick));
        Lemma_Advance_Position
         (Initial_Positions (Item), Initial_Velocities (Item), Old_Tick);
        pragma
         Assert
